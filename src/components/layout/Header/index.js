...
export const Header = ({
  ...
  useEffect(() => {
    const handleUserProfileUpdate = (event) => {
      logger.info('🔄 Header: Received profile update event', event.detail);
      ...
      try {
        ...
        if (typeof refreshUser === 'function') {
          ...
          logger.info('✅ Header: User data synced via localStorage');
        }
      } catch (error) {
        logger.error('❌ Header: Error syncing profile update:', error);
      }
    };
    ...
    const handleStorageChange = (event) => {
      if (event.key === 'currentUser' && event.newValue) {
        try {
          ...
          logger.info('🔄 Header: Detected localStorage user update', updatedUser);
          ...
          logger.error('❌ Header: Error parsing updated user data:', error);
        }
      }
    };
  ...
  const handleQuickEditSave = (updatedData) => {
    logger.info('✅ Header: Quick edit saved', updatedData);
