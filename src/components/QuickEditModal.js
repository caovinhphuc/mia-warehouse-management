...
const QuickEditModal = ({ isOpen, onClose, user, onSave }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      logger.info("🔄 QuickEdit: Updating user profile...");
      ...
      if (result.success) {
        logger.info("✅ QuickEdit: Profile updated successfully");
        ...
        try {
          ...
          if (currentUser) {
            ...
            logger.info("✅ QuickEdit: Updated localStorage");
          }
        } catch (e) {
          logger.warn(
            "⚠️ QuickEdit: Could not update localStorage:",
            e.message
          );
        }
      ...
      logger.error("❌ QuickEdit: Error updating profile:", error);
      setError(
        error.message || "Lỗi khi cập nhật thông tin. Vui lòng thử lại."
      );
    } finally {
      setLoading(false);
    }
